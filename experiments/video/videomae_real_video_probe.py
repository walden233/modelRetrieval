...
try:
    print("\nLoading real videos...")
    # 加载两个内容完全不同的视频
    video_sports = load_video("dataset/whirl_videos/human/hat.mp4")
    video_cooking = load_video("dataset/whirl_videos/robot/hat.mp4")
    
    # 为了模拟相似视频，我们可以对 sports.mp4 做一点轻微的修改，比如只取稍微不同的帧
    vr_sports = VideoReader("dataset/whirl_videos/human/hat.mp4", ctx=cpu(0))
    ...
    print(f"Similarity between a human video and a robot video: {similarity_sports_vs_cooking:.4f}")
